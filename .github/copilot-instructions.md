...
- Projects: `src/MsgBakMan.App` (WPF + CommunityToolkit.Mvvm + ModernWpfUI + Microsoft.Extensions.Hosting/Logging), `src/MsgBakMan.Core` (models/normalization/fingerprinting), `src/MsgBakMan.Data` (SQLite+Dapper+migrations), `src/MsgBakMan.ImportExport` (streaming XML + media), `installer/MsgBakMan.Installer` (WiX MSI, built separately, not in `.sln`).
- `src/MsgBakMan.WinUI` exists on disk as an empty scaffold but is **not in the solution** — ignore it.

## Entry points
- UI wiring: `src/MsgBakMan.App/MainWindow.xaml` + `MainWindow.xaml.cs` (sets `DataContext = new MainViewModel()`). `MainWindow` also hosts About menu → `AboutWindow`.
...
- Settings persistence: `AppSettingsStore` reads/writes `%LOCALAPPDATA%\MsgBakMan\settings.json` (theme + accent palette).
- Note: App.csproj sets `UseWindowsForms=true` solely for `FolderBrowserDialog`.

## Project folder & DB lifecycle
- Default project folder: `%USERPROFILE%\Documents\MsgBakManProject`.
...
- Dapper raw SQL repos live in `src/MsgBakMan.Data/Repositories/*.cs` (no EF): `ImportRepository`, `ExportRepository`, `ConversationRepository`, `ConversationMaintenance`, `PhoneNormalizationMaintenance`.
...
- After imports and merges, `ConversationMaintenance.BackfillConversations()` rebuilds conversation groupings.
...
- Two media-export modes: flat (`ExportMedia` — SHA256-named files) and by-phone (`ExportMediaByPhone` — subfolders per conversation/phone number).
- Full-text search: `SearchMessages` command searches across all messages in `MainViewModel`.
...
- Merge suggestions auto-detect AU `+61`↔`0` format duplicates; bulk or single accept via `MainViewModel`.
- `RepairPhoneNumbers` command runs `PhoneNormalizationMaintenance.RepairLegacyPlus10Numbers()` (accessible from About window).
...
- CI enforces whitespace via `dotnet-format` global tool (v5.1.250801): `dotnet format whitespace .\MsgBakMan.sln --verify-no-changes --no-restore`.
